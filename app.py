...
def process_report_file(path: Path, import_id: int) -> tuple[int, str]:
    ...
    for _, r in df.iterrows():
        ...
        # Segurança adicional para Uber TVDE: quando o nome do cabeçalho mudou
        # e o mapeamento não encontrou a coluna, procura automaticamente os
        # campos configurados de reembolso/saldo pago ao motorista.
        if "TVDE" in norm(origin) and abs(reimb) < 0.000001:
            mapped_targets = {
                canonical_column(mapping.get(k, ""))
                for k in ("reimbursement_1","reimbursement_2","reimbursement_3","reimbursement_4")
                if mapping.get(k) and mapping.get(k) != "-"
            }
            fallback_values = []
            for c in r.index:
                cc = canonical_column(c)
                if not cc:
                    continue
                is_mapped_like = any(t and (t == cc or t in cc or cc in t) for t in mapped_targets)
                is_reimbursement_like = any(token in norm(c) for token in ("REEMBOLSO", "AJUSTE", "SALDO DA VIAGEM", "PAGO A SI"))
                # Não reutiliza a coluna principal de valor bruto.
                if (is_mapped_like or is_reimbursement_like) and canonical_column(c) != canonical_column(mapping.get("value_column", "")):
                    fallback_values.append(money(r[c]))
            if fallback_values:
                reimb = sum(fallback_values)
...
def dashboard():
    with db() as con:
        ...
        if last:
            ...
            r = con.execute("""WITH category_base AS (
                    SELECT driver_id,
                        CASE WHEN origins LIKE 'TVDE |%' THEN 'TVDE' ELSE 'DELIVERY' END AS category,
                        SUM(gross) gross, SUM(cash) cash, SUM(commission) commission,
                        MAX(fuel) fuel, MAX(discount) discount,
                        SUM(reimbursement) reimbursement, MAX(immediate) immediate,
                        SUM(net_before_group-bank_fee) net,
                        MAX(iban) iban
                    FROM closing_items WHERE closing_id=?
                    GROUP BY driver_id, CASE WHEN origins LIKE 'TVDE |%' THEN 'TVDE' ELSE 'DELIVERY' END
                ), driver_base AS (
                    SELECT driver_id,
                        SUM(gross) gross, SUM(cash) cash, SUM(commission) commission,
                        MAX(fuel) fuel, MAX(discount) discount,
                        SUM(reimbursement) reimbursement, MAX(immediate) immediate,
                        SUM(net) net, MAX(iban) iban
                    FROM category_base GROUP BY driver_id
                )
                SELECT COALESCE(SUM(gross),0) gross,
                    ...
                    COALESCE(SUM(cash),0) cash, COALESCE(SUM(net),0) net,
                ...
                FROM driver_base""", (last["id"],)).fetchone()
            stats = dict(r)
...
def manager_report():
    with db() as con:
        ...
        cid = closing["id"]
        raw = con.execute("""WITH category_base AS (
                SELECT driver_id, CASE WHEN origins LIKE 'TVDE |%' THEN 'TVDE' ELSE 'DELIVERY' END AS category,
                    SUM(gross) gross, SUM(cash) cash, SUM(commission) commission,
                    MAX(fuel) fuel, MAX(discount) discount,
                    SUM(reimbursement) reimbursement, MAX(immediate) immediate,
                    SUM(net_before_group-bank_fee) net, MAX(iban) iban
                FROM closing_items WHERE closing_id=?
                GROUP BY driver_id, CASE WHEN origins LIKE 'TVDE |%' THEN 'TVDE' ELSE 'DELIVERY' END
            ), driver_base AS (
                SELECT driver_id, SUM(gross) gross, SUM(cash) cash,
                    MAX(fuel) fuel, MAX(discount) discount, SUM(reimbursement) reimbursement,
                    SUM(net) net, MAX(iban) iban
                FROM category_base GROUP BY driver_id
            )
            SELECT COALESCE(SUM(gross),0) gross, COALESCE(SUM(net),0) net,
                COALESCE(SUM(fuel),0) fuel, COALESCE(SUM(discount),0) discount,
                COALESCE(SUM(reimbursement),0) reimbursement, COALESCE(SUM(cash),0) cash,
                COUNT(DISTINCT CASE WHEN net<0 THEN driver_id END) negatives,
                COUNT(DISTINCT CASE WHEN TRIM(COALESCE(iban,''))='' THEN driver_id END) missing_iban,
                COALESCE((SELECT SUM(gross) FROM category_base WHERE category='TVDE'),0) tvde,
                COALESCE((SELECT SUM(gross) FROM category_base WHERE category='DELIVERY'),0) delivery
            FROM driver_base""",(cid,)).fetchone()
        stats=dict(raw)
