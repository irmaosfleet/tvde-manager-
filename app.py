...
def manager_report():
    with db() as con:
        ...
        missing=con.execute("""SELECT ci.driver_name,ci.origins,COALESCE(d.partner,'') partner,
            ci.net_before_group-ci.bank_fee net FROM closing_items ci LEFT JOIN drivers d ON d.id=ci.driver_id
            WHERE ci.closing_id=? AND TRIM(COALESCE(ci.iban,''))='' ORDER BY ci.driver_name""",(cid,)).fetchall()
    return render_template("manager.html",closing=closing,stats=stats,partners=partners,negatives=negatives,missing=missing)
