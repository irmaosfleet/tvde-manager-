...
def process_report_file(path: Path, import_id: int) -> tuple[int, str]:
    ...
    for _, r in df.iterrows():
        ...
        if "EATS" in norm(origin):
            ...
            cash = 0.0
        # Somente campos cujo nome configurado contém explicitamente REEMBOLSO.
        # Ajustes após a viagem e impostos não são tratados como reembolso.
        reimb = 0.0
        if "TVDE" in norm(origin):
            for key in ("reimbursement_1", "reimbursement_2", "reimbursement_3", "reimbursement_4"):
                configured = str(mapping.get(key, "") or "")
                if "REEMBOLSO" in norm(configured):
