...
def process_report_file(path: Path, import_id: int) -> tuple[int, str]:
    ...
    for _, r in df.iterrows():
        ...
        # Reembolso: usa somente as quatro colunas explicitamente configuradas
        # na aba Arquivos_Pgto. Nunca tenta adivinhar por palavras como
        # "Pago a si" ou "Saldo", pois essas colunas podem conter o bruto.
...
def build_closing(label: str) -> int:
    ...
    try:
        ...
        for a in ordered_aggregated:
            ...
            discount = money(d.get("discount")) if use_fixed else 0.0
            immediate = money(d.get("immediate")) if use_fixed else 0.0
            if use_fixed:
...
def dashboard():
    with db() as con:
        ...
        last = con.execute("SELECT * FROM closings WHERE status LIKE 'PROCESSADO%' ORDER BY id DESC LIMIT 1").fetchone()
        stats = {"gross":0,"commission":0,"partner_commission":0,"fuel":0,"discount":0,"reimbursement":0,"cash":0,"net":0,"payments":0,"missing_iban":0}
        origins = []
        fuel_breakdown = []
        if last:
            cid = last["id"]
            row = con.execute("""SELECT
                ...
                COUNT(DISTINCT CASE WHEN TRIM(COALESCE(iban,''))<>'' THEN origins||'|'||iban END) payments,
                COUNT(DISTINCT CASE WHEN TRIM(COALESCE(iban,''))='' THEN driver_id END) missing_iban
                FROM closing_items WHERE closing_id=?""", (cid,)).fetchone()
            stats.update(dict(row))
            shares = con.execute("""SELECT
                COALESCE(SUM(ci.commission * CASE WHEN COALESCE(d.commission_owner,0)>1 THEN d.commission_owner/100.0 ELSE COALESCE(d.commission_owner,0) END),0) owner_total,
                COALESCE(SUM(ci.commission * CASE WHEN COALESCE(d.partner_commission,0)>1 THEN d.partner_commission/100.0 ELSE COALESCE(d.partner_commission,0) END),0) partner_total
                FROM closing_items ci JOIN drivers d ON d.id=ci.driver_id WHERE ci.closing_id=?""", (cid,)).fetchone()
            stats["commission"] = float(shares["owner_total"] or 0)
            stats["partner_commission"] = float(shares["partner_total"] or 0)
            origins = con.execute("SELECT origins,SUM(gross) total FROM closing_items WHERE closing_id=? GROUP BY origins ORDER BY total DESC", (cid,)).fetchall()
...
def missing_iban_page():
    ...
    with db() as con:
        ...
        closing_missing = 0
        if last:
            closing_missing = con.execute("SELECT COUNT(*) FROM closing_items WHERE closing_id=? AND TRIM(COALESCE(iban,''))=''", (last["id"],)).fetchone()[0]
        where = "TRIM(COALESCE(iban,''))=''"
        params = []
        if q:
            like = f"%{q}%"
            where += " AND (name LIKE ? OR external_id LIKE ? OR partner LIKE ? OR city LIKE ?)"
            params.extend([like, like, like, like])
        rows = con.execute(f"SELECT * FROM drivers WHERE {where} ORDER BY name LIMIT 500", params).fetchall()
...
def save_missing_iban(driver_id: int):
    ...
    with db() as con:
        ...
        if not driver:
            return "Motorista não encontrado", 404
        con.execute("UPDATE drivers SET iban=?,bank_color=?,partner=?,partner_commission=?,updated_at=? WHERE id=?", (iban, bank_color, partner, partner_commission, datetime.now().isoformat(timespec="seconds"), driver_id))
        closings = con.execute("SELECT DISTINCT closing_id FROM closing_items WHERE driver_id=? AND TRIM(COALESCE(iban,''))=''", (driver_id,)).fetchall()
        con.execute("UPDATE closing_items SET iban=?,bank_color=? WHERE driver_id=? AND TRIM(COALESCE(iban,''))=''", (iban, bank_color, driver_id))
    for row in closings:
        refresh_closing_payment_groups(row["closing_id"])
    flash(f"IBAN de {driver['name']} atualizado e fechamento recalculado.", "success")
    return redirect(url_for("missing_iban_page"))
...
def manager_report():
    with db() as con:
        ...
        raw = con.execute("""SELECT
            ...
            COUNT(DISTINCT CASE WHEN TRIM(COALESCE(iban,''))='' THEN driver_id END) missing_iban,
            COALESCE(SUM(CASE WHEN origins LIKE 'TVDE |%' THEN gross ELSE 0 END),0) tvde,
            COALESCE(SUM(CASE WHEN origins LIKE 'DELIVERY |%' THEN gross ELSE 0 END),0) delivery
            FROM closing_items WHERE closing_id=?""", (cid,)).fetchone()
        stats = dict(raw)
        commissions = con.execute("""SELECT
            COALESCE(SUM(ci.commission * CASE WHEN COALESCE(d.commission_owner,0)>1 THEN d.commission_owner/100.0 ELSE COALESCE(d.commission_owner,0) END),0) commission,
            COALESCE(SUM(ci.commission * CASE WHEN COALESCE(d.partner_commission,0)>1 THEN d.partner_commission/100.0 ELSE COALESCE(d.partner_commission,0) END),0) partner_commission
            FROM closing_items ci JOIN drivers d ON d.id=ci.driver_id WHERE ci.closing_id=?""", (cid,)).fetchone()
        stats["commission"] = float(commissions["commission"] or 0)
        stats["partner_commission"] = float(commissions["partner_commission"] or 0)
        partners = con.execute("""SELECT COALESCE(NULLIF(TRIM(d.partner),''),'SEM PARCEIRO') partner,
            COUNT(DISTINCT ci.driver_id) drivers,
            COALESCE(SUM(ci.gross),0) gross,
            COALESCE(SUM(ci.commission * CASE WHEN COALESCE(d.partner_commission,0)>1 THEN d.partner_commission/100.0 ELSE COALESCE(d.partner_commission,0) END),0) partner_commission,
            COALESCE(SUM(ci.net_before_group-ci.bank_fee),0) net
            FROM closing_items ci LEFT JOIN drivers d ON d.id=ci.driver_id
            WHERE ci.closing_id=? GROUP BY COALESCE(NULLIF(TRIM(d.partner),''),'SEM PARCEIRO') ORDER BY gross DESC""", (cid,)).fetchall()
        negatives = con.execute("""SELECT driver_name, GROUP_CONCAT(origins, ' / ') origins,
            COALESCE(MAX(d.partner),'') partner, SUM(ci.net_before_group-ci.bank_fee) net
            FROM closing_items ci LEFT JOIN drivers d ON d.id=ci.driver_id
            WHERE ci.closing_id=? GROUP BY ci.driver_id HAVING net<0 ORDER BY net""", (cid,)).fetchall()
        missing = con.execute("""SELECT driver_name, GROUP_CONCAT(origins, ' / ') origins,
            COALESCE(MAX(d.partner),'') partner, SUM(ci.net_before_group-ci.bank_fee) net
            FROM closing_items ci LEFT JOIN drivers d ON d.id=ci.driver_id
            WHERE ci.closing_id=? AND TRIM(COALESCE(ci.iban,''))='' GROUP BY ci.driver_id ORDER BY driver_name""", (cid,)).fetchall()
        fuel_breakdown = con.execute("""SELECT source_file, COALESCE(SUM(amount),0) total
            FROM fuel GROUP BY source_file ORDER BY total DESC""").fetchall()
