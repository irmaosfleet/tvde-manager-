    ...
    try:
        ...
        for e in earnings:
            try:
                ...
                if "EATS" in norm(e.get("origin_ref")):
                    a["cash"] += abs(row_cash)
        ...
        for a in aggregated.values():
            ...
            # O dinheiro em mãos participa da base da comissão na Uber Eats,
            # mas é abatido no final porque já está com o motorista.
            if is_eats:
                net = (gross + cash) - commission - fuel_value - discount - immediate + reimbursement - cash
            else:
...
def closing_xml(closing_id: int):
    ...
    if len(parts) == 1:
        xml = generate_xml_part(closing_id, parts[0], 1)
        return send_file(io.BytesIO(xml), as_attachment=True, download_name=f"pagamentos_{closing_id}_parte_01.xml", mimetype="application/xml")
    out = io.BytesIO()
    with zipfile.ZipFile(out, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        for index, part in enumerate(parts, 1):
            zf.writestr(f"pagamentos_{closing_id}_parte_{index:02d}.xml", generate_xml_part(closing_id, part, index))
    out.seek(0)
    return send_file(out, as_attachment=True, download_name=f"pagamentos_{closing_id}_{len(parts)}_partes.zip", mimetype="application/zip")
